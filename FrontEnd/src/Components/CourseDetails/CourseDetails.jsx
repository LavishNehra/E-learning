...
const CourseDetails = () => {
  ...
  const deleteUnitHandler = async (unitId) => {
    ...
    if (isConfirmed) {
      try {
        const response = await fetch(
          `${Front_ENV.Back_Origin}/deleteUnit/${unitId}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!data.error) {
          ...
          showMessage(data.message || "Unit deleted successfully", false);
        } else {
          showMessage(data.error, true);
        }
      } catch (error) {
        showMessage("Error deleting unit", true);
        console.error(error);
      }
    }
  };
  ...
  const editUnitHandler = (unitId) => {
    navigate(`/EditUnit/${unitId}`, { state: { courseId: id } });
  };
                        ...
                        <div style={{ paddingRight: "60px" }}>
                          ...
                          >
                            ...
                            >
                              ...
                              <span>{unit.chaptersCount} Modules</span>
                            </div>
                            ...
                            >
                              ...
                              <span>{unit.questionsCount} Quizzes</span>
                            </div>
                          </div>
                          ...
                          {/* Edit/Delete buttons for instructors */}
